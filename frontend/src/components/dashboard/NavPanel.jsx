...
function NavPanel({ expanded, setExpanded }) {
    ...
    const navItems = pages.map((page, i) => {
        return <Link key={i} to={`${page.toLowerCase()}`}
    ...
    return (
        // Outer Div
        <div
            className={"h-lvh p-2 pr-0 fixed transition-all duration-500 md:sticky md:top-0 text-neutral-500 row-span-2 z-10 " + (expanded ? 'left-0  w-[200px]' : '-left-full w-[90px]')}>
            {/*Inner Section*/}
            <div
                ...
                {/*Logo*/}
                <div className="w-full flex flex-col items-center min-h-[30%] gap-3">
